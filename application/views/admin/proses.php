...
<section class="card panel-card border-0 shadow-sm mb-4">
    <div class="card-body p-4">
        <div class="d-flex justify-content-between align-items-center mb-3">
            <div>
                <span class="text-primary fw-bold fs-11 text-uppercase tracking-wider">Daftar Siklus Evaluation</span>
            ...
            <table id="tablePeriodePenilaian" class="table table-hover table-bordered align-middle dt-responsive nowrap w-100">
                <thead class="table-light">
                    <tr>
                        ...
                        <th>Nama Periode</th>
                        <th>Siklus & Tahun</th>
                        <th class="text-center">Terpenilai</th>
                        <th>Kandidat Terbaik (Rank #1)</th>
                        <th class="text-center">Status TOPSIS</th>
                ...
                <tbody>
                    <?php if (!empty($periode_penilaian_list)): ?>
                        <?php $no = 1;
                        foreach ($periode_penilaian_list as $row): ?>
                            <tr>
                                ...
                                <td>
                                    <strong class="d-block text-dark fs-13">
                                        <?= html_escape($row['nama_periode']); ?>
                                    </strong>
                                    <small class="text-muted fs-11">Dikalkulasi pada
                                        <?= date('d M Y H:i', strtotime($row['tanggal_kalkulasi'])); ?>
                                    </small>
                                </td>
                                <td>
                                    <span class="badge bg-primary-subtle text-primary border border-primary-subtle px-2 py-1 fs-11 text-uppercase me-1">
                                        <?= html_escape($row['jenis_periode']); ?>
                                    </span>
                                    <span class="fw-semibold text-dark fs-13">
                                        <?= html_escape($row['tahun']); ?>
                                    </span>
                                </td>
                                <td class="text-center">
                                    <span class="badge bg-info-subtle text-info border border-info-subtle px-2 py-1 fs-11">
                                        <i class="ti ti-users me-1"></i>
                                        <?= $row['jumlah_terpenilai']; ?> Orang
                                    </span>
                                </td>
                                <td>
                                    <strong class="d-block text-dark fs-13">
                                        <i class="ti ti-trophy text-warning me-1"></i>
                                        <?= html_escape($row['pemenang_nama']); ?>
                                    </strong>
                                    <small class="text-muted fs-11">
                                        <?= html_escape($row['pemenang_kategori']); ?> • V =
                                        <?= number_format($row['skor_tertinggi'], 4); ?>
                                    </small>
                                </td>
                                <td class="text-center">
                                ...
                                <td class="text-center">
                                    <div class="d-flex align-items-center justify-content-center gap-1">
                                        <a href="<?= site_url('proses/detail/' . $row['id_periode']); ?>" class="btn btn-sm btn-brand p-1 px-2"
                                            title="Lihat Detail Hasil TOPSIS Pegawai">
