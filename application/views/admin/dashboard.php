            ...
            <div style="max-width: 680px;">
                <div class="d-flex align-items-center gap-2 mb-2">
                    <span class="badge bg-warning text-dark font-bold px-2.5 py-1 fs-11 tracking-wider text-uppercase">
                        ...
                        <?= html_escape(!empty($settings['satker']['nama_satker']) ? $settings['satker']['nama_satker'] : 'Pengadilan Negeri Lubuk Pakam Kelas I-A'); ?>
                    </span>
                ...
                <p class="text-white text-opacity-80 fs-14 leading-relaxed mb-0">
                    Aplikasi BeRewards menggunakan metode <strong>TOPSIS (Technique for Order of Preference by Similarity to Ideal
    ...
    <div class="col-md-4 col-xl-2">
        <a href="<?= site_url('pegawai'); ?>" class="card kpi-card border-0 shadow-sm text-decoration-none h-100">
            <div class="card-body p-3 text-center">
                ...
                <h4 class="fw-bold mb-0 text-dark"><?= isset($kpi['total_pegawai']) ? (int) $kpi['total_pegawai'] : 0; ?> <span
                        class="fs-11 text-muted fw-normal">Orang</span></h4>
            </div>
        </a>
    </div>
    <div class="col-md-4 col-xl-2">
        <a href="<?= site_url('kriteria'); ?>" class="card kpi-card border-0 shadow-sm text-decoration-none h-100">
            <div class="card-body p-3 text-center">
                ...
                <h4 class="fw-bold mb-0 text-dark"><?= isset($kpi['total_kriteria']) ? (int) $kpi['total_kriteria'] : 0; ?> <span
                        class="fs-11 text-muted fw-normal">Item</span></h4>
            </div>
        </a>
    </div>
    <div class="col-md-4 col-xl-2">
        <a href="<?= site_url('periode'); ?>" class="card kpi-card border-0 shadow-sm text-decoration-none h-100">
            <div class="card-body p-3 text-center">
                ...
                <h4 class="fw-bold mb-0 text-dark"><?= isset($kpi['periode_aktif']) ? (int) $kpi['periode_aktif'] : 0; ?> <span
                        class="fs-11 text-muted fw-normal">Siklus</span></h4>
            </div>
        </a>
    </div>
    <div class="col-md-4 col-xl-2">
    ...
    <div class="col-md-4 col-xl-2">
        <a href="<?= site_url('laporan'); ?>" class="card kpi-card border-0 shadow-sm text-decoration-none h-100">
            <div class="card-body p-3 text-center">
                ...
                <h4 class="fw-bold mb-0 text-dark"><?= isset($kpi['ba_disahkan']) ? (int) $kpi['ba_disahkan'] : 0; ?> <span
                        class="fs-11 text-muted fw-normal">Resmi</span></h4>
            </div>
        </a>
    </div>
    <div class="col-md-4 col-xl-2">
        <a href="<?= site_url('audit'); ?>" class="card kpi-card border-0 shadow-sm text-decoration-none h-100">
            <div class="card-body p-3 text-center">
                ...
                <h4 class="fw-bold mb-0 text-dark"><?= isset($kpi['audit_log']) ? (int) $kpi['audit_log'] : 0; ?> <span
                        class="fs-11 text-muted fw-normal">Log</span></h4>
            </div>
        </a>
    </div>
</section>

<!-- Spotlight Section: Penerima Reward Teratas Per Kategori (Diambil dari Hasil Perhitungan TOPSIS Terakhir) -->
    ...
    <div class="card-body p-4">
        <div class="d-flex justify-content-between align-items-center mb-3">
            <div>
                ...
                <h4 class="fw-bold text-dark mb-0">Penerima Reward Teratas Per Kategori
                    (<?= html_escape(!empty($active_periode_label) ? $active_periode_label : 'Triwulan II ' . date('Y')); ?>)</h4>
            </div>
                    ...
                    <div class="col-md-6 col-xl-3">
                        <div class="card border rounded-3 p-3 bg-light h-100 shadow-sm position-relative overflow-hidden">
                            <div class="d-flex align-items-center justify-content-between mb-2">
                                <?php if (!empty($winner['is_calculated'])): ?>
                                    <span class="badge bg-warning text-dark font-bold px-2 py-1 fs-10">
                                        <i class="ti ti-trophy text-warning me-1"></i>RANK #1
                                    </span>
                                    <div class="d-flex align-items-center gap-1">
                                        <span class="badge bg-success-subtle text-success border border-success-subtle px-1.5 py-0.5 fs-10"
                                            title="Hasil Resmi Perhitungan TOPSIS Engine"><i class="ti ti-check me-0.5"></i>TOPSIS</span>
                                        <span
                                            class="badge bg-primary-subtle text-primary border border-primary-subtle px-2 py-1 fs-11"><?= html_escape($winner['kategori']); ?></span>
                                    </div>
                                <?php else: ?>
                                    <span class="badge bg-secondary-subtle text-secondary px-2 py-1 fs-10">
                                        <i class="ti ti-clock me-1"></i>Belum Dihitung
                                    </span>
                                    <span class="badge bg-light text-muted border px-2 py-1 fs-11"><?= html_escape($winner['kategori']); ?></span>
                                <?php endif; ?>
                            </div>

                            <?php if (!empty($winner['is_calculated'])): ?>
                                <?php
                                $foto_src = (!empty($winner['foto']) && strpos($winner['foto'], 'http') === 0)
                                    ? $winner['foto']
                                    : base_url(!empty($winner['foto']) ? $winner['foto'] : 'assets/icons/logo.png');
                                ?>
                                <div class="text-center my-2">
                                    <img src="<?= $foto_src; ?>" alt="<?= html_escape($winner['nama']); ?>"
                                        class="rounded-circle border border-2 border-primary shadow-sm mb-2" style="width: 64px; height: 64px; object-fit: cover;"
                                        onerror="this.src='<?= base_url('assets/icons/logo.png'); ?>';">
                                    <h6 class="fw-bold text-dark mb-0 fs-13 text-truncate" title="<?= html_escape($winner['nama']); ?>">
                                        <?= html_escape($winner['nama']); ?>
                                    </h6>
                                    <small class="text-muted fs-11 d-block text-truncate">NIP. <?= html_escape($winner['nip']); ?></small>
                                </div>
                                <div class="p-2 bg-white rounded text-center border mt-2">
                                    <div class="d-flex justify-content-between align-items-center mb-1">
                                        <small class="text-muted fs-10">Skor Preferensi TOPSIS</small>
                                        <small class="text-muted fs-10 text-truncate" style="max-width: 110px;"
                                            title="<?= html_escape($winner['periode']); ?>"><?= html_escape($winner['periode']); ?></small>
                                    </div>
                            ...
                            <?php else: ?>
                                <div class="text-center my-3 py-1">
                                    <div class="avatar-placeholder rounded-circle bg-secondary bg-opacity-10 d-inline-flex align-items-center justify-content-center mb-2"
                                        style="width: 64px; height: 64px;">
                                        <i class="ti ti-user-search fs-24 text-muted"></i>
                                    </div>
                                    <h6 class="fw-semibold text-muted mb-0 fs-12">Menunggu Penilaian</h6>
                                    <small class="text-muted fs-11 d-block opacity-75">Sesi belum dikalkulasi</small>
                                </div>
                                <div class="p-2 bg-white rounded text-center border mt-2">
                                    <small class="text-muted fs-10 d-block mb-1">Status Kategori</small>
                                    <a href="<?= site_url('proses'); ?>" class="btn btn-sm btn-outline-primary py-0.5 px-2 fs-11">
                                        <i class="ti ti-calculator me-1"></i>Proses TOPSIS
                                    </a>
                                </div>
                            <?php endif; ?>
                        </div>
                ...
                <div class="activity-list">
                    <?php if (!empty($recent_activities)): ?>
                        <?php foreach ($recent_activities as $act): ?>
                            <?php
                            $time_display = date('H:i', strtotime($act['timestamp']));
                            if (date('Y-m-d', strtotime($act['timestamp'])) !== date('Y-m-d')) {
                                $time_display = date('d M, H:i', strtotime($act['timestamp']));
                            }

                            // Dynamic action styling
                            $action_type = !empty($act['tipe_aksi']) ? strtoupper($act['tipe_aksi']) : 'ACTIVITY';
                            $icon_class = 'ti ti-activity';
                            $bg_class   = 'bg-primary-subtle text-primary';

                            if ($action_type === 'INSERT') {
                                $icon_class = 'ti ti-plus';
                                $bg_class   = 'bg-success-subtle text-success';
                            } elseif ($action_type === 'UPDATE') {
                                $icon_class = 'ti ti-edit';
                                $bg_class   = 'bg-warning-subtle text-warning';
                            } elseif ($action_type === 'DELETE') {
                                $icon_class = 'ti ti-trash';
                                $bg_class   = 'bg-danger-subtle text-danger';
                            } elseif ($action_type === 'AUTH' || strpos($act['modul'], 'Autentikasi') !== FALSE) {
                                $icon_class = 'ti ti-shield-lock';
                                $bg_class   = 'bg-info-subtle text-info';
                            } elseif ($action_type === 'REPORT' || strpos($act['modul'], 'Laporan') !== FALSE) {
                                $icon_class = 'ti ti-file-text';
                                $bg_class   = 'bg-primary-subtle text-primary';
                            }
                            ?>
                            <div class="activity-item d-flex align-items-center justify-content-between py-2.5 pb-3 border-bottom border-light">
                                <div class="d-flex align-items-center gap-3">
                                    <span class="activity-icon <?= $bg_class; ?> p-2 rounded-3 d-inline-flex">
                                        <i class="<?= $icon_class; ?> fs-16"></i>
                                    </span>
                                    <div>
                                        <div class="d-flex align-items-center gap-1.5 mb-0.5">
                                            <strong class="text-dark fs-13 d-block"><?= html_escape($act['modul']); ?></strong>
                                            <?php if (!empty($act['username'])): ?>
                                                <span class="badge bg-light text-muted border px-1.5 py-0.5 fs-10"><?= html_escape($act['username']); ?></span>
                                            <?php endif; ?>
                                        </div>
                                        <p class="text-muted fs-11 mb-0"
                                            style="max-width: 270px; text-overflow: ellipsis; overflow: hidden; white-space: nowrap;"
                                            title="<?= html_escape($act['aktivitas']); ?>">
                                            <?= html_escape($act['aktivitas']); ?>
                                        </p>
                                    </div>
                                </div>
                                <small class="text-muted fs-11 text-nowrap ms-2"><?= $time_display; ?></small>
                            </div>
                        <?php endforeach; ?>
                    <?php else: ?>
                        <div class="text-center py-4 text-muted">
                            <i class="ti ti-inbox fs-28 d-block mb-1 opacity-50"></i>
                            <span class="fs-12">Belum ada aktivitas tercatat</span>
                        </div>
                    <?php endif; ?>
                </div>
            </div>
