...
<aside class="col-lg-4">
    ...
    <div class="card border-0 shadow-sm mb-4 overflow-hidden position-relative" style="background: linear-gradient(135deg, #1E1B4B 0%, #312E81 100%);">
        <div class="card-body p-4 text-white position-relative">
            <div class="d-flex justify-content-between align-items-center mb-3">
                ...
                <span class="badge bg-white bg-opacity-20 text-white border border-white border-opacity-25 fs-10">OFFICIAL ID</span>
            </div>
                    ...
                    <?php foreach ($activity_logs as $index => $log): ?>
                        <div class="d-flex align-items-start gap-3 py-2 <?= ($index < count($activity_logs) - 1) ? 'border-bottom border-light' : ''; ?>">
                            ...
                            <div class="w-100">
                                ...
                                <small class="text-muted fs-11 d-block"><i class="ti ti-clock me-1"></i><?= date('d M Y H:i', strtotime($log['waktu'])); ?></small>
                                <small class="text-muted fs-11 d-block"><i class="ti ti-device-desktop me-1"></i><?= html_escape($log['perangkat']); ?> &bull; <?= html_escape($log['ip']); ?></small>
                            </div>
                        </div>
                    <?php endforeach; ?>
                <?php endif; ?>
